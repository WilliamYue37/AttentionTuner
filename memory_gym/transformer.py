...
class ImageEmbedding(nn.Module):
    def __init__(self, d_model):
        ...
        self.fc = nn.Sequential(
            nn.Linear(3136, d_model),  # Adjust input size based on your CNN architecture
            nn.Tanh()
        )
...
class Transformer(nn.Module):
    def __init__(self, action_dim, d_model, nhead, num_layers, feed_forward_dim=2048):
        super().__init__()
        self.action_dim = action_dim
        self.d_model = d_model
