...
class Trainer():
    def __init__(self, model, train_dataset, test_dataset, args):
        ...
        self.even_split = args.even_mem_split
    ...
    def visualize_attention(self, attention_weights, ground_truth_attention_heads):
        ...
        for head in range(self.viz_num_heads):
            ground_truth_attention_weights = ground_truth_attention_heads[head] if self.even_split else ground_truth_attention_heads[0]
            ...
            for i, label in enumerate(labels):
                if i % 5 != 1:  # Hide every other label
        ...
        plt.tight_layout()
        plt.savefig(str(self.attention_heatmap_folder / f'final.pdf'), bbox_inches='tight')
        plt.close()
    ...
    def get_test_accuracy(self):
        ...
        correct, total = 0, 0
        with torch.no_grad():
            for batch_idx, (obs, actions, pad_mask, attention_pad_mask, *correct_attention_heads) in enumerate(self.test_dl):
    ...
    def train(self, num_epoch):
        ...
        for _ in range(num_epoch):
            ...
            correct, total = 0, 0
            attention_weights, correct_attention_heads = [], [] # for visualization
            for batch_idx, (obs, actions, pad_mask, attention_pad_mask, *correct_attention_heads) in enumerate(self.dl):
                self.opt.zero_grad()
                obs, actions, pad_mask, attention_pad_mask = obs.cuda(), actions.cuda(), pad_mask.cuda(), attention_pad_mask.cuda()
                for i in range(len(correct_attention_heads)):
                    correct_attention_heads[i] = correct_attention_heads[i].cuda()
                output, _, attention_weights = self.model(obs, actions)
                ...
                for layer, head in self.memory_loss_heads:
                    correct_attention_weights = correct_attention_heads[head - 1] if self.even_split else correct_attention_heads[0]
            ...
            if self.epoch % self.visualize_every == 0:
                self.visualize_attention(attention_weights, correct_attention_heads)
